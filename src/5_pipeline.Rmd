...
## European `pvol` {#baltrad-pvol-pipeline}
...
## European `pvol` to `vp` {#baltrad-vp-pipeline}
...
## `vp` archiving {#enram-vp-pipeline}

**European** `vp` data are archived daily by a pipeline running on Amazon Web Services (AWS) which transfers `vp` files from the BALTRAD FTP server to a public [S3 bucket](https://lw-enram.s3-eu-west-1.amazonaws.com). This pipeline is maintained by the [Open science lab for biodiversity][oscibio] and its code and documentation are available in [this repository](https://github.com/enram/data-repository). Note that any issues in the pipeline that are not resolved within 2 days (the time `vp` data are kept on the FTP server) **can result in data loss**.
...
Not yet defined, but likely to be part of the ENRAM pipeline (see \@ref(enram-vp-pipeline)).
